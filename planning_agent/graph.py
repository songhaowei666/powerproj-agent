...
# 文件下载 URL 基址，与 Agent Card 中 planning-agent 服务地址一致
PLANNING_FILE_BASE_URL = "http://localhost:8001"
...
def _build_file_download_url(file_id: str, fm: FileManager) -> str:
    """构造文件下载 URL。"""
    return fm.build_download_url(file_id, PLANNING_FILE_BASE_URL)


def _build_file_markdown_link(file_name: str, file_id: str, fm: FileManager) -> str:
    """构造 Markdown 可点击下载链接行。"""
    url = _build_file_download_url(file_id, fm)
    return f"- [{file_name}]({url})"


def _build_file_artifact(file_name: str, file_id: str, fm: FileManager) -> Dict[str, Any]:
    """构造 file 类型 artifact，供主控透传下载链接。"""
    return {
        "type": "file",
        "file": {
            "uri": _build_file_download_url(file_id, fm),
            "name": file_name,
        },
    }
...
def build_planning_graph(
    ...
    async def execute_action(state: PlanningState) -> PlanningState:
        ...
        if intent == "query_project":
            ...
            upload_artifacts: List[Dict[str, Any]] = []
            for f in state.pending_files:
                staging_file_id = f.get("staging_file_id")
                if staging_file_id:
                    file_id, file_path = fm.commit_staging_file(
                        str(staging_file_id),
                        state.matched_project.project_code,
                        state.node_code,
                    )
                ...
                effective_file_id = db.add_file_record(
                    project_code=state.matched_project.project_code,
                    node_code=state.node_code,
                    file_id=file_id,
                    file_name=file_name,
                    file_path=f"{state.matched_project.project_code}/{state.node_code}/{file_name}",
                )
                uploaded_links.append(
                    _build_file_markdown_link(file_name, effective_file_id, fm)
                )
                upload_artifacts.append(
                    _build_file_artifact(file_name, effective_file_id, fm)
                )

            node_name = _get_node_name(state.node_code)
            state.result_text = (
                f"成功上传 {len(uploaded_links)} 个文件到「{node_name}」节点：\n"
                + "\n".join(uploaded_links)
            )
            state.artifacts.extend(upload_artifacts)
            ...
            for f in files:
                ...
                file_url = _build_file_download_url(f["file_id"], fm)
                file_lines.append(
                    f"- [{f['file_name']}]({file_url})"
                    f" ({node_name}) - 上传时间：{f['created_at']}"
                )
                file_artifacts.append(
                    _build_file_artifact(f["file_name"], f["file_id"], fm)
                )
            listing_text = (
                "为您找到以下文件：\n"
                + "\n".join(file_lines)
            ...
            state.result_text = listing_text
            state.artifacts = [{"type": "text", "text": listing_text}, *file_artifacts]
