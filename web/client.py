...
def chat_response_to_dict(chat_resp: ChatResponse) -> Dict[str, Any]:
    """将 ChatResponse 转为可 JSON 序列化的字典。"""
    confirmation_dict: Optional[Dict[str, Any]] = None
    if chat_resp.confirmation:
        confirmation_dict = {
            "action": chat_resp.confirmation.action,
            "title": chat_resp.confirmation.title,
            "options": chat_resp.confirmation.options,
            "confirm_type": chat_resp.confirmation.confirm_type,
            "body": chat_resp.confirmation.body,
        }
    return {
        "task_id": chat_resp.task_id,
        "context_id": chat_resp.context_id,
        "state": chat_resp.state,
        "text": chat_resp.text,
        "artifacts": chat_resp.artifacts,
        "invocation_traces": chat_resp.invocation_traces,
        "is_error": chat_resp.is_error,
